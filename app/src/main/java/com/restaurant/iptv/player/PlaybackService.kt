...
class PlaybackService : Service(), PlaybackCommands {
    ...
    override fun onCreate() {
        ...
        // Load the EPG on startup and keep it fresh. We only retain ~36h of
        // programmes to bound memory, so a TV left running for days would
        // otherwise end up with an empty guide and no way to reload it.
        scope.launch {
            while (true) {
    ...
    /** Set when the compat HLS URL keeps failing, so we go back to the raw
     *  stream instead of retrying a URL this provider doesn't serve. */
    private var hlsRewriteFailed = false

    private fun loadIntoPlayer(ch: ChannelEntity) {
        val p = player ?: return
        // Compat: use the HLS container instead of raw TS — a different demux
        // path that avoids the code TCL firmware tends to crash in. If that URL
        // turns out not to exist, hlsRewriteFailed sends us back to the original.
        val url = if (compat && !hlsRewriteFailed && ch.streamUrl.endsWith(".ts"))
    ...
    private fun requestRecovery(reason: String) {
        ...
        recoveryJob = scope.launch {
            ...
            while (currentChannel != null) {
                ...
                // If the compat HLS URL isn't working, stop insisting on it.
                if (compat && !hlsRewriteFailed && localTry >= HLS_FALLBACK_AFTER) {
                    Log.w(TAG, "HLS rewrite failing — falling back to the original stream URL")
                    hlsRewriteFailed = true
                }
    ...
    companion object {
        ...
        private const val HLS_FALLBACK_AFTER = 3
        private const val EPG_REFRESH_MS = 6 * 60 * 60_000L
