...
class PlaybackService : Service(), PlaybackCommands {
    ...
    private fun buildPlayer() {
        ...
        // try the next one instead of dying). Compat additionally prefers the
        // software decoders outright — slower, but immune to driver crashes.
        ...
        if (compat) {
            renderers.setMediaCodecSelector { mimeType, requiresSecureDecoder, requiresTunnelingDecoder ->
                androidx.media3.exoplayer.mediacodec.MediaCodecUtil
                    ...
                    .sortedBy { info ->
                        if (info.name.startsWith("OMX.google") || info.name.startsWith("c2.android")) 0 else 1
                    }
            }
        }
