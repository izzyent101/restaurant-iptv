...
class WebServer(
    ...
    // --- Brute-force protection: 5 wrong passwords → that IP is locked 15 min ---
    private val fails = java.util.concurrent.ConcurrentHashMap<String, Pair<Int, Long>>()

    private fun lockedMs(ip: String): Long {
        val e = fails[ip] ?: return 0
        return if (e.first >= MAX_FAILS && e.second > System.currentTimeMillis())
            e.second - System.currentTimeMillis() else 0
    }

    private fun recordFail(ip: String) {
        val now = System.currentTimeMillis()
        val e = fails[ip]
        val count = if (e == null || now > e.second) 1 else e.first + 1
        fails[ip] = count to (now + LOCK_MS)
    }

    private fun clearFails(ip: String) { fails.remove(ip) }

    private suspend fun io.ktor.server.application.ApplicationCall.respondLocked(remaining: Long) {
        val min = ((remaining + 59_999) / 60_000).coerceAtLeast(1)
        respondText(
            json.encodeToString(ApiResult(false, "Too many attempts — locked for $min more minute(s).")),
        ...
        engine = embeddedServer(CIO, port = port, host = "0.0.0.0") {
            ...
            intercept(ApplicationCallPipeline.Plugins) {
                ...
                if (path.startsWith("/api/")) {
                    ...
                    val remaining = lockedMs(ip)
                    if (remaining > 0) {
                        call.respondLocked(remaining)
                    } else {
                        // Only an actively WRONG key counts as an attempt — a missing
                        // or empty header (fresh page load, logged-out UI) doesn't.
                        if (!call.request.headers["X-Access-Key"].isNullOrEmpty()) recordFail(ip)
                        call.respondText(
            ...
            routing {
                ...
                // --- Compatibility mode (per-TV gentler decode path) ---
                get("/api/compat") {
                    call.respondJson("{\"on\":${prefs.compatMode()}}")
                }
                post("/api/compat") {
                    val on = call.receiveParameters()["on"] == "true"
                    prefs.setCompatMode(on)
                    commands.cmdApplyCompat()
                    call.respondJson(json.encodeToString(ApiResult(true,
                        if (on) "Compatibility mode ON — playback restarting" else "Compatibility mode OFF — playback restarting")))
                }
                ...
                post("/api/login") {
                    val ip = call.request.origin.remoteHost
                    val remaining = lockedMs(ip)
                    if (remaining > 0) return@post call.respondLocked(remaining)
                    ...
                    if (key.isNotEmpty() && pw == key) {
                        ...
                        recordFail(ip)
                        val nowLocked = lockedMs(ip)
                        if (nowLocked > 0) {
                            call.respondLocked(nowLocked)
                        } else {
                            call.respondText(
                ...
                get("/api/groups") {
                    ...
                    val list = if (prov == null) emptyList() else {
                        ...
                        repo.orderGroups(prov.id, repo.getGroups(prov.id).sorted())
                            .map { GroupDto(it, hidden.contains(it)) }
                    }
                    call.respondJson(json.encodeToString(list))
                }
                // Save a custom category order (newline-separated full list).
                post("/api/groups/order") {
                    val prov = repo.getActiveProvider()
                        ?: return@post call.respondJson(json.encodeToString(ApiResult(false, "no provider")))
                    val order = call.receiveParameters()["groups"]
                        ?.split("\n")?.map { it.trim() }?.filter { it.isNotEmpty() } ?: emptyList()
                    repo.setGroupOrder(prov.id, order)
                    call.respondJson(json.encodeToString(ApiResult(true)))
                }
    ...
    companion object {
        ...
        private const val LOCK_MS = 15 * 60_000L
    }
}
