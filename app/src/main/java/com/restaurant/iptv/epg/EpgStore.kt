...
object EpgStore {
    ...
    /** All programmes for a channel (sorted by start), or empty. */
    fun programmes(providerId: Long, channelId: String?): List<Programme> {
        if (channelId.isNullOrBlank()) return emptyList()
        return data[providerId]?.get(channelId) ?: emptyList()
    }
