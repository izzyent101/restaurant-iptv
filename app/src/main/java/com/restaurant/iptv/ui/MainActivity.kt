...
class MainActivity : AppCompatActivity() {
    ...
    private fun showBrowser() {
        ...
        var pos = -1
        if (cur != null) {
            for (i in 0 until channelAdapter.size()) {
                if (channelAdapter.itemAt(i)?.channel?.id == cur.id) { pos = i; break }
            }
        }
        if (pos >= 0) {
            ...
            binding.channelList.post {
                binding.channelList.findViewHolderForAdapterPosition(p)?.itemView?.requestFocus()
    ...
    private fun showHistory() {
        ...
        binding.historyList.post {
            binding.historyList.findViewHolderForAdapterPosition(0)?.itemView?.requestFocus()
                ?: binding.historyClear.requestFocus()
        }
    }
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        ...
        when (keyCode) {
            ...
            KeyEvent.KEYCODE_DPAD_LEFT -> when {
                hist -> return super.onKeyDown(keyCode, event)   // horizontal strip handles it
                !open -> { showBrowser(); return true }
                binding.categoryList.hasFocus() -> return true    // already leftmost
                binding.channelList.hasFocus() -> {
                    if (cats) binding.categoryList.requestFocus() else openCategories()
                    return true
                }
            }
            KeyEvent.KEYCODE_DPAD_RIGHT -> when {
                hist -> return super.onKeyDown(keyCode, event)
                binding.categoryList.hasFocus() -> { binding.channelList.requestFocus(); return true }
            }
            KeyEvent.KEYCODE_DPAD_UP -> if (!open && !hist) { surf(-1); return true }
            KeyEvent.KEYCODE_DPAD_DOWN -> if (!open && !hist) { surf(1); return true }
