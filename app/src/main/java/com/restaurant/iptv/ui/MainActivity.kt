...
class MainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        super.onResume()
        // onCreate already loads; without this guard every launch did the whole
        // channel/category rebuild twice.
        if (android.os.SystemClock.uptimeMillis() - lastLoadAt > 1_500) loadData()
    }

    // ---------------- Data ----------------

    private fun loadData() {
        lastLoadAt = android.os.SystemClock.uptimeMillis()
    ...
    private var categoryGen = 0

    /** Builds the row list on a background thread (filtering thousands of channels
     *  plus a guide lookup each was running on the UI thread). Stale results from
     *  rapid category changes are discarded via the generation counter. */
    private fun showCategory(name: String, onReady: (() -> Unit)? = null) {
        val pid = providerId ?: return
        val gen = ++categoryGen
        binding.channelHeader.text = name
        val source = allVisible
        val favs = favorites
        lifecycleScope.launch {
            val list = withContext(Dispatchers.Default) {
                val chans = when (name) {
                    CAT_ALL -> source
                    CAT_FAV -> source.filter { favs.contains(it.streamKey) }
                    else -> source.filter { it.groupTitle == name }
                }
                chans.map { ch ->
                    val (now, next) = EpgStore.nowNext(pid, ch.epgChannelId)
                    ChannelUi(ch, now, next, favs.contains(ch.streamKey))
                }
            }
            if (gen != categoryGen) return@launch          // a newer request won
            channelAdapter.submit(list)
            binding.channelCount.text = if (list.isEmpty()) "" else "${list.size} channels"
            onReady?.invoke()
        }
    }
    ...
    private fun showBrowser() {
        ...
        showCategory(categoryAdapter.current() ?: CAT_ALL) {
            // Scroll to + focus the playing channel once the rows exist.
    ...
    private fun showInfoBar(ch: ChannelEntity) {
        ...
        if (!ch.logoUrl.isNullOrBlank()) {
            binding.infoLogo.load(ch.logoUrl) { size(160); crossfade(false) }
        } else binding.infoLogo.setImageDrawable(null)
