...
class RichChannelAdapter(
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        if (!ch.logoUrl.isNullOrBlank()) {
            // Decode to the view's size, not the source's. Provider logos are often
            // 512px+; decoding those full-res for a 50dp row is pure waste.
            holder.logo.load(ch.logoUrl) { size(140); crossfade(false) }
