...
class Repository(context: Context) {
    ...
    suspend fun addTv(name: String, address: String, mac: String? = null) {
        val addr = normalizeAddress(address)
        val current = getTvs().filterNot { it.address == addr }
        saveTvs(current + TvEndpoint(name.trim().ifEmpty { addr }, addr, mac?.trim()?.takeIf { it.isNotEmpty() }))
    }
    ...
    // ---------- Custom category order (per provider, stored in prefs) ----------

    private suspend fun orderMap(): MutableMap<String, List<String>> =
        runCatching {
            json.decodeFromString<Map<String, List<String>>>(prefs.groupOrderJson()).toMutableMap()
        }.getOrElse { mutableMapOf() }

    suspend fun setGroupOrder(providerId: Long, order: List<String>) {
        val map = orderMap()
        map[providerId.toString()] = order
        prefs.setGroupOrderJson(json.encodeToString(map as Map<String, List<String>>))
    }

    /** Apply the saved custom order: ordered names first, any new/unknown groups after. */
    suspend fun orderGroups(providerId: Long, groups: List<String>): List<String> {
        val order = orderMap()[providerId.toString()] ?: return groups
        val set = groups.toSet()
        return order.filter { set.contains(it) } + groups.filter { !order.contains(it) }
    }
