...
class Prefs(private val context: Context) {
    ...
    private val keyOverlayPrompted = booleanPreferencesKey("overlay_prompted")
    private val keyGroupOrder = stringPreferencesKey("group_order_json")
    private val keyCompatMode = booleanPreferencesKey("compat_mode")
    private val keyCompatSet = booleanPreferencesKey("compat_mode_set")
    private val keyBootStamps = stringPreferencesKey("boot_stamps")
    ...
    // --- Compatibility mode (gentler decode path for fragile TV firmware) ---
    suspend fun compatMode(): Boolean =
        context.dataStore.data.map { it[keyCompatMode] ?: false }.first()

    suspend fun setCompatMode(on: Boolean) {
        context.dataStore.edit {
            it[keyCompatMode] = on
            it[keyCompatSet] = true
        }
    }

    /** Whether compat mode was ever explicitly chosen (user/web/auto-detect). */
    suspend fun compatModeSet(): Boolean =
        context.dataStore.data.map { it[keyCompatSet] ?: false }.first()

    /** Recent boot timestamps (CSV of epoch millis) for crash-loop detection. */
    suspend fun bootStamps(): List<Long> =
        context.dataStore.data.map {
            (it[keyBootStamps] ?: "").split(",").mapNotNull { s -> s.toLongOrNull() }
        }.first()

    suspend fun setBootStamps(stamps: List<Long>) {
        context.dataStore.edit { it[keyBootStamps] = stamps.joinToString(",") }
    }

    // --- Custom category order (JSON map: providerId -> [group names]) ---
    suspend fun groupOrderJson(): String =
        context.dataStore.data.map { it[keyGroupOrder] ?: "{}" }.first()

    suspend fun setGroupOrderJson(json: String) {
        context.dataStore.edit { it[keyGroupOrder] = json }
    }

    // --- One-time "Display over other apps" prompt (auto-start on boot) ---
    suspend fun overlayPrompted(): Boolean =
        context.dataStore.data.map { it[keyOverlayPrompted] ?: false }.first()

    suspend fun setOverlayPrompted() {
        context.dataStore.edit { it[keyOverlayPrompted] = true }
    }
    ...
    companion object {
        ...
        const val DEFAULT_REPO = "izzyent101/restaurant-iptv"
    }
}
