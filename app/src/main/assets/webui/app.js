...
function renderGroupManage() {
  ...
  const move = (g, dir) => {
    ...
    const j = i + dir;
    if (j < 0 || j >= allGroups.length) return;
    allGroups.splice(i, 1);
    allGroups.splice(j, 0, g);
    saveOrder();
    renderGroupManage();
  ...
  shown.forEach((g) => {
    ...
    const mk = (txt, dir) => {
      ...
      b.className = 'ordBtn';
      b.textContent = txt;
      b.onclick = (e) => { e.preventDefault(); e.stopPropagation(); move(g, dir); };
      return b;
    };
    // Reordering only makes sense without a search filter (arrows move within the full list).
    if (!q) { right.appendChild(mk('▲', -1)); right.appendChild(mk('▼', 1)); }
