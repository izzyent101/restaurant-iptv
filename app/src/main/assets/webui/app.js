...
async function api(path, opts) {
  ...
  if (sentKey) opts.headers['X-Access-Key'] = sentKey;   // never send an empty key
  let res = await fetch(path, opts);
  if (res.status === 401) {
    // Another call may have just prompted and stored the key — reuse it silently.
    let key = ak();
    if (key === sentKey) {
      const pw = window.prompt('This TV is password-protected. Enter the control password:');
      if (pw != null) { localStorage.setItem('ak', pw); key = pw; }
    }
    if (key && key !== sentKey) {
      opts.headers['X-Access-Key'] = key;
