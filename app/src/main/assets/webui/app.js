...
function renderArrange() {
  ...
  const move = (g, dir) => {
    const i = vis.indexOf(g);
    const j = i + dir;
    if (j < 0 || j >= vis.length) return;
    vis.splice(i, 1);
    vis.splice(j, 0, g);
    // Rebuild the master order: shown categories in the arranged order,
    // hidden ones after (their order is irrelevant while hidden).
