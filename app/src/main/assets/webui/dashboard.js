...
function ak() { return localStorage.getItem('ak') || sessionStorage.getItem('ak') || ''; }
function akHeaders() { const k = ak(); return k ? { 'X-Access-Key': k } : {}; }
function promptKey() {
  const pw = window.prompt('A TV is password-protected. Enter the control password (must match on all TVs):');
  if (pw != null) localStorage.setItem('ak', pw);
}

function form(data) {
  const body = new URLSearchParams();
  Object.keys(data).forEach((k) => { if (data[k] != null) body.append(k, data[k]); });
  return { method: 'POST', headers: Object.assign({ 'Content-Type': 'application/x-www-form-urlencoded' }, akHeaders()), body };
}
let promptedOnce = false;
async function jget(url) {
  const sentKey = ak();
  let r = await fetch(url, { cache: 'no-store', headers: akHeaders() });
  if (r.status === 401) {
    // Only ONE prompt per page load — parallel calls reuse the stored key.
    if (ak() === sentKey && !promptedOnce) { promptedOnce = true; promptKey(); }
    if (ak() !== sentKey || promptedOnce) r = await fetch(url, { cache: 'no-store', headers: akHeaders() });
  }
  return r.json();
}
...
function buildCard(tv) {
  ...
  card.querySelector('.act-reconnect').onclick = () => fetch(base(tv) + '/api/retry', { method: 'POST', headers: akHeaders() }).catch(() => {});
  ...
  card.querySelector('.act-stop').onclick = () => fetch(base(tv) + '/api/stop', { method: 'POST', headers: akHeaders() }).catch(() => {});
  card.querySelector('.act-refresh').onclick = async (e) => {
    e.target.textContent = '…';
    await fetch(base(tv) + '/api/refresh', { method: 'POST', headers: akHeaders() }).catch(() => {});
...
$('#bulkStop').onclick = () => { tvs.forEach((tv) => fetch(base(tv) + '/api/stop', { method: 'POST', headers: akHeaders() }).catch(() => {})); };
$('#bulkReconnect').onclick = () => { tvs.forEach((tv) => fetch(base(tv) + '/api/retry', { method: 'POST', headers: akHeaders() }).catch(() => {})); };
  ...
  tvs.forEach((tv) => {
    ...
    if (v && v.updateAvailable) {
      ...
      btn.onclick = async () => {
        ...
        btn.textContent = '…';
        await fetch(base(tv) + '/api/update/install', { method: 'POST', headers: akHeaders() }).catch(() => {});
...
$('#fleetCheck').onclick = async () => {
  ...
  await Promise.all(tvs.map((tv) =>
    fetch(base(tv) + '/api/update/check', { method: 'POST', headers: akHeaders() }).catch(() => {})
  ));
...
$('#fleetInstall').onclick = async () => {
  ...
  await Promise.all(ready.map((tv) =>
    fetch(base(tv) + '/api/update/install', { method: 'POST', headers: akHeaders() }).catch(() => {})
  ));
