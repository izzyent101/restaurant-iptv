...
async function pollTv(tv) {
  ...
  try {
    ...
    // Health line: what is this TV doing, for how long, and is it stuck?
    const hp = card.querySelector('.tvhealth');
    const dur = fmtDur(s.secondsInState);
    const ch = s.channelName ? '"' + s.channelName + '"' : 'nothing';
    card.classList.remove('alert');
    if (s.state === 'playing') {
      health[tv.address] = 'ok';
      hp.className = 'tvhealth hOk';
      hp.textContent = '✓ Live on ' + ch + (dur ? ' for ' + dur : '');
    } else if (s.state === 'recovering' || s.state === 'error') {
      health[tv.address] = 'bad';
      hp.className = 'tvhealth hBad';
      hp.textContent = '✗ STUCK on ' + ch + (dur ? ' — reconnecting for ' + dur : '') +
        (s.retryCount ? ' (' + s.retryCount + ' tries)' : '') +
        (s.lastError ? ' · ' + s.lastError : '');
      card.classList.add('alert');
    } else if (s.state === 'buffering') {
      const long = (s.secondsInState || 0) > 45;
      health[tv.address] = long ? 'bad' : 'warn';
      hp.className = 'tvhealth ' + (long ? 'hBad' : 'hWarn');
      hp.textContent = (long ? '✗ Buffering too long on ' : '⚠ Buffering ') + ch + (dur ? ' for ' + dur : '');
      if (long) card.classList.add('alert');
    } else if (s.state === 'no_provider') {
      health[tv.address] = 'bad';
      hp.className = 'tvhealth hBad';
      hp.textContent = '✗ No provider set up';
      card.classList.add('alert');
    } else {
      health[tv.address] = 'warn';
      hp.className = 'tvhealth hWarn';
      hp.textContent = 'Idle — nothing playing' + (dur ? ' for ' + dur : '');
    }
    updateSummary();
  } catch (e) {
    ...
    const hp = card.querySelector('.tvhealth');
    hp.className = 'tvhealth hBad';
    hp.textContent = '✗ Offline — TV asleep, app closed, or network issue';
