...
async function openBrowse() {
  ...
  $('#bulkMsg').textContent = 'Click a channel to play it on ' + ($('#browseSelOnly').checked ? 'the selected TVs' : 'ALL TVs') + '.';
...
function renderBrowseList() {
  ...
  browseChannels
    ...
    .forEach((c) => {
      ...
      el.onclick = () => bulkPlayChannel(c, el);
      list.appendChild(el);
    });
}

async function bulkPlayChannel(c, el) {
  const targets = $('#browseSelOnly').checked ? selectedTvs() : tvs;
  if (!targets.length) { $('#bulkMsg').textContent = 'Tick at least one TV, or untick "Selected TVs only".'; return; }
  el.classList.add('playing');
  $('#bulkMsg').textContent = 'Playing "' + c.name + '"…';
  const want = c.name.toLowerCase();
  let ok = 0, miss = 0, off = 0;
  for (const tv of targets) {
    try {
      const chans = await ensureChannels(tv);
      const match = chans.find((x) => x.name.toLowerCase() === want) ||
                    chans.find((x) => x.name.toLowerCase().includes(want));
      if (match) { await fetch(base(tv) + '/api/play', form({ channelId: match.id })); ok++; }
      else miss++;
    } catch (e) { off++; }
  }
  $('#bulkMsg').textContent = '"' + c.name + '" → playing on ' + ok + ' TV(s)' +
    (miss ? ', ' + miss + ' had no match' : '') + (off ? ', ' + off + ' offline' : '') + '.';
  setTimeout(pollAll, 500);
  setTimeout(() => el.classList.remove('playing'), 3000);
}
